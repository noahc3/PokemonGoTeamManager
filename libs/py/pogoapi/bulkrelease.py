...
if __name__ == '__main__':
    ...
    #    logging.critical("Error! Session data missing! Please relaunch!")
    #    sys.exit(-1)

    #oldSession = open("session.txt", "r+")
    ...
    #session = poko_session.reauthenticate(oldSession)
    ...
    if session:
        ...
        uuids = open("releaseUUIDs.txt", "r+")
        uuid_lines = uuids.readlines()

        line_count = sum(1 for line in uuid_lines)

        print("IDs found: ", line_count)
        i = 0
        while i != line_count:
            curUUID = uuid_lines[i].strip('\n')
            print("Current UUID: ", curUUID)
            print("Current Line: ", i)
            try:
                session.releasePokemonById(int(curUUID))
                ...
                try:
                    ...
                    error = c
                    print(error, "Failed to transfer Pokemon by UUID with BULK. Locking down.")
                    errorLog = open("error.txt", "w+")
                    errorLog.write("Errored! Bulk transfer FAILED!!")
                    errorLog.write("\n")
                    errorLog.write(error)
                    errorLog.write("\n")
                    errorLog.write("Attempted UUID Release: ", curUUID)
                    errorLog.close()
